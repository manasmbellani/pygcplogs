...
TOPIC_NAME = "sumologic"
SUBSCRIPTION_NAME = "sumologic"
REQUESTS_TIMEOUT = 3
SLEEP_PURGE = 10
...
def delete_subscription(sub_name):
    """Deleting the subscription with given name

        Args:
            sub_name (str): Subscription name
    
        Returns:
            bool: Whether the subscription was successfully deleted
    """
    was_sub_deleted = True
    project_id = get_project_id()
    if project_id:
        print(f"[*] Deleting subscription: {sub_name} via subscription client...")
        with pubsub_v1.SubscriberClient() as subscriber:
            sub_path = subscriber.subscription_path(project_id, sub_name)
            subscriber.delete_subscription(request={"subscription": sub_path})
            was_sub_deleted = True

    return was_sub_deleted
...
def check_sumologic_env_vars_exist():
    """
    Checks whether the Sumologic HEC environment vars are defined such as SUMOLOGIC_URL

        Returns:
            bool: True, if both SUMOLOGIC_URL are defined. 
    """
    sumologic_url = os.getenv("SUMOLOGIC_URL")
    return (sumologic_url)
...
def process_single_message(message):
    ...
    try:
        ...
        send_to_sumologic(message)
    except Exception as e:
        print(f"[-] Exception processing message. Details: {e.__class__}, {str(e)}")
    finally:
        message.ack()

def send_to_sumologic(message, data_error_handling='ignore', ssl_verify=False):
    """Sends messages to Sumologic HTTP Hosted Collector based on URL, token defined in OS env vars, 
    SUMOLOGIC_URL 
        ...
        Returns:
            bool: True, if message sent successfully to Sumologic. False, otherwise.
    """
    message_sent = False
    try:
        sumologic_url = os.getenv("SUMOLOGIC_URL")
        body = json.loads(message.data.decode(errors=data_error_handling))
        
        resp = requests.post(sumologic_url, json=body, verify=ssl_verify, allow_redirects=True,
        ...
        print(f"[-] Exception sending message to Sumologic. Error: {e.__class__}, {str(e)}")
    
    return message_sent

def main():
    sumologic_env_vars_defined = check_sumologic_env_vars_exist()
    if not sumologic_env_vars_defined:
        print(f"[-] Sumologic environment variables not defined")
        return 1
